...
import { sortObjectsByDateAsc, sortObjectsByDateDesc } from './datetime'
...
export async function getRelatedProjects(category, projectId, count = 3) {
    ...
    if (category) {
        ...
        const filtered = projects.filter(({ projectId: id }) => id !== projectId)
...
export async function getMoreProjects(count) {
    let more_projects, _projects;

    const { projects } = await getAllProjects()
    const sorted = sortObjectsByDateAsc(projects)
    _projects = sorted.slice(0, count)

    more_projects = _projects.map((project) => ({
        title: project.title,
        slug: project.slug,
