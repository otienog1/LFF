...
const PaymentInfo = ({ cardNumber, cardType, expiryMonthYear, cvcInput, refreshCardTypeIcon, handleCreditCardNumberKey, handleExpiry, handleExpiryPaste, refreshExpiryMonthValidation, cvn, handleCvn, name, handleName, alert }) => {
    ...
    //         env: 'production',
    //         hosted_button_id: 'LMA3KTLR9SDT2',
    //         image: {
    //             src: 'https://pics.paypal.com/00/s/YjQ3ZmVlYWYtNTE1OC00NGVhLWJmMzQtNTE3ZDYzOGYyMzc3/file.PNG',
    ...
    return (
        <>
            ...
            {/* <div className="flex mb-8 space-x-16">
                <label className={`${active == `card` ? `bg-lff_400` : ''} flex border-2 border-solid border-lff_700 rounded items-center w-1/2 p-4 justify-between cursor-pointer`}>
                    ...
                    <input type="radio" className="form-radio text-lff_800" name="radio" value="1" onChange={() => setActive('card')} />
                </label>
                <label className={`${active == `paypal` ? `bg-lff_400` : ''} flex border-2 border-solid border-lff_700 rounded items-center w-1/2 p-4 justify-between cursor-pointer`}>
                    <span className="flex items-center">
                        <span>
